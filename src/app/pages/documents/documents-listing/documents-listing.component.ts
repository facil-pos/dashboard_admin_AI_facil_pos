...
export class DocumentsListingComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  roleModel: IResModel = { id: 0, name: '', docs: [] };
  ...
  constructor(private apiService: DocumentService, private cdr: ChangeDetectorRef, private renderer: Renderer2, private modalService: NgbModal) { }
  ...
  ngOnInit(): void {
    this.roles$ = this.apiService.getDocs();
  }

  edit(id: 1 | 2 | 3) {
    this.role$ = this.apiService.getDoc(id);
    this.role$.subscribe((user: IResModel | undefined) => {
      if (user) {
        this.roleModel = user;
      }
    });
  ...
  onSubmit(event: Event, myForm: NgForm) {
    ...
    const updateFn = () => {
      this.apiService.updateDocs(this.roleModel.id, this.roleModel).subscribe({
        next: () => {
    ...
    const createFn = () => {
      this.apiService.createDocs(this.roleModel).subscribe({
        next: () => {
          this.showAlert(successAlert);
          this.reloadEvent.emit(true);
        },
